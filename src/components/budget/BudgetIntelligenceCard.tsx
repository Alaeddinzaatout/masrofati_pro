...
const BudgetIntelligenceCard = ({ stats }: BudgetIntelligenceCardProps) => {
  ...
  const fetchAiAdvice = async () => {
    ...
    setLoading(true);
    try {
      const systemPrompt = `أنت "شرطي الميزانية" الليبي الصارم.
مهمتك تحليل معدل الصرف اليومي وإعطاء رسالة (جملتين كحد أقصى) للمستخدم بلهجة ليبية.
إذا كان الصرف ممتازاً امدحه، وإذا كان معدل الحرق عالياً وسيتجاوز الميزانية قم بتوبيخه بطريقة مضحكة وقاسية.
